...
const seedDatabase = async () => {
  try {
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      $push: {
        familiares: {
          $each: [familiares[0]._id, familiares[1]._id]
        }
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[1]._id, {
      $push: {
        familiares: familiares[2]._id
      }
    });
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      resumenMedico: resumenesMedicos[0]._id
    });
    await Usuario.findByIdAndUpdate(usuarios[1]._id, {
      resumenMedico: resumenesMedicos[1]._id
    });
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      resumenMedico: resumenesMedicos[2]._id
    });
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      $push: {
        antecedentes: {
          $each: [antecedentes[0]._id, antecedentes[1]._id]
        }
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      $push: {
        antecedentes: {
          $each: [antecedentes[2]._id, antecedentes[3]._id]
        }
      }
    });
    ...
    // Crear Dosis
    const dosisData = [{
        medicamento: medicamentos[3]._id,
        cantidadDiaria: 1,
      ...
      {
        medicamento: medicamentos[4]._id,
        cantidadDiaria: 2,
      ...
      {
        medicamento: medicamentos[0]._id,
        cantidadDiaria: 3,
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      $push: {
        formulas: formulas[0]._id
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      $push: {
        formulas: formulas[1]._id
      }
    });
    ...
    const recordatoriosData = [{
        ...
        horasToma: ['08:00'],
        frecuencia: 'Diaria',
        descripcionToma: 'Tomar en ayunas con un vaso de agua',
        activo: true
      },
      {
        usuario: usuarios[2]._id,
        ...
        frecuencia: 'Diaria',
        descripcionToma: 'Tomar con las comidas',
        activo: true
      },
      {
        usuario: usuarios[1]._id,
        medicamento: medicamentos[0]._id,
        horasToma: ['10:00'],
        frecuencia: 'Dias especificos',
        diasEspecificos: [1, 4],
        descripcionToma: 'Tomar si presenta dolor de cabeza',
        activo: false
      }
    ];
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      $push: {
        recordatorios: recordatorios[0]._id
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      $push: {
        recordatorios: recordatorios[1]._id
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[1]._id, {
      $push: {
        recordatorios: recordatorios[2]._id
      }
    });
    ...
    const registrosData = [{
        usuario: usuarios[0]._id,
        medicamento: medicamentos[3]._id,
        fechaHoraToma: new Date(haceDosDias.setHours(8, 3, 0)),
        descripcion: 'Tomado según lo programado'
      },
      {
        usuario: usuarios[0]._id,
        medicamento: medicamentos[3]._id,
        fechaHoraToma: new Date(ayer.setHours(8, 5, 0))
      },
      {
        usuario: usuarios[0]._id,
        medicamento: medicamentos[3]._id,
        fechaHoraToma: new Date(ahora.setHours(8, 2, 0))
      },
      {
        usuario: usuarios[2]._id,
        medicamento: medicamentos[4]._id,
        fechaHoraToma: new Date(ayer.setHours(8, 10, 0)),
        descripcion: 'Tomado con el desayuno'
      },
      {
        usuario: usuarios[2]._id,
        medicamento: medicamentos[4]._id,
        fechaHoraToma: new Date(ayer.setHours(20, 5, 0)),
        descripcion: 'Tomado con la cena'
      },
      {
        usuario: usuarios[2]._id,
        medicamento: medicamentos[4]._id,
        fechaHoraToma: new Date(ahora.setHours(8, 0, 0))
      },
      {
        usuario: usuarios[1]._id,
        medicamento: medicamentos[0]._id,
        fechaHoraToma: new Date(ahora.setHours(10, 15, 0)),
        descripcion: 'Dolor de cabeza leve'
      }, // María tomó uno hoy
    ];
    ...
    await Usuario.findByIdAndUpdate(usuarios[0]._id, {
      $push: {
        historialConsumo: {
          $each: [registrosConsumo[0]._id, registrosConsumo[1]._id, registrosConsumo[2]._id]
        }
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      $push: {
        historialConsumo: {
          $each: [registrosConsumo[3]._id, registrosConsumo[4]._id, registrosConsumo[5]._id]
        }
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[1]._id, {
      $push: {
        historialConsumo: registrosConsumo[6]._id
      }
    });


    /// Crear Domiciliarios
    const hashPasswordDomi = await bcrypt.hash('domipass123', 10); // Contraseña para domiciliarios
    const domiciliariosData = [{
        nombre: 'Roberto',
        apellidos: 'Sánchez Torres',
        cedula: '7777777777',
        placaVehiculo: 'ABC123',
        contrasena: hashPasswordDomi, // <-- AÑADIR CONTRASEÑA
      },
      {
        nombre: 'Diana',
    ...
    const pedidosData = [{
        ...
        medicamentos: [{
          ...
          medicamento: medicamentos[2]._id,
          cantidad: 1
        }],
      ...
      },
      {
        ...
        medicamentos: [{
          ...
          medicamento: medicamentos[1]._id,
          cantidad: 1
        }],
    ...
    await Usuario.findByIdAndUpdate(usuarios[2]._id, {
      $push: {
        pedidos: pedidos[1]._id
      }
    });
    await Usuario.findByIdAndUpdate(usuarios[1]._id, {
      $push: {
        pedidos: pedidos[2]._id
      }
    });
    await Domiciliario.findByIdAndUpdate(domiciliarios[0]._id, {
      $push: {
        pedidosAsignados: pedidos[0]._id
      }
    });
    await Domiciliario.findByIdAndUpdate(domiciliarios[1]._id, {
      $push: {
        pedidosAsignados: pedidos[1]._id
      }
    });
